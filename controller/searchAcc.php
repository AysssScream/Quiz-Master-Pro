    ...
    <img src="MainPics/h2.png" alt="QuizMaster Pro Logo" style="max-height: 250px; margin-top: 0; width: 1300px; margin-left: 20px">
        ...
        <div class="card-container">
            ...
            if (isset($_POST['search']) && !empty(trim($_POST['search']))) {
                ...
                $sql = "SELECT * FROM user WHERE role != 'admin' AND (BINARY username LIKE '%$searchTerm%' OR firstname LIKE '%$searchTerm%' OR lastname LIKE '%$searchTerm%')";

                $result = $conn->query($sql);
                $c = 1;
                // Rest of your HTML and PHP code for displaying results...
            ...
            ?>
                        <div class="account-card">
                            <div class="card-content">
                                <span class="card-title">Account #<?= $c ?></span>
                                <p><strong>Role:</strong> <?= $row["role"] ?></p>
                                <p><strong>Username:</strong> <?= $row["username"] ?></p>
                                <p><strong>Name:</strong> <?= ucwords($row["firstname"] . ' ' . $row["middlename"] . ' ' . $row["lastname"]) ?></p>
